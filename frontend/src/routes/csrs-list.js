...
export default function CSRsList(){
  ...
  return (
    <React.Fragment>
      ...
      { opError !== null && <AlertBar setMessage={setOpError} message={opError} type= "error"/>}
      { opCorrect !== null && <AlertBar setMessage={setOpCorrect} message={opCorrect} type="success"/>}
      { (error === null && !isLoaded && csrs !== null) && <CircularProgress/>}
      { (error === null && isLoaded && csrs !== null) && 
        <Grid container spacing={2}>
          {csrs.length > 1 ?
            (csrs.map(csr => (
                <CSRBox key={csr.id.toString()} csr={csr} setOpError={setOpError} setOpCorrect={setOpCorrect} onCSRUpdate={getCSRS}/>
            ))) : (
              <CSRBox key={csrs.id.toString()} csr={csrs} setOpError={setOpError} setOpCorrect={setOpCorrect} onCSRUpdate={getCSRS}/>
            )}
        </Grid>
      }
    </React.Fragment>
