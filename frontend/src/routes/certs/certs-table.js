...
import { Table, TableContainer, TablePagination, Paper, CircularProgress, Container, Button, Box } from '@material-ui/core';
...
export default function CertsTable() {
    ...
    const emptyRows = rowsPerPage - Math.min(rowsPerPage, crts.length - page * rowsPerPage);

    return (
        <React.Fragment>
            { error !== null && <AlertBar setMessage={setError} message={error} type= "error"/>}
            { correctRevoke !== null && <AlertBar setMessage={setCorrectRevoke} message={correctRevoke} type="success"/>}
            { revokeError !== null && <AlertBar setMessage={setRevokeError} message={revokeError} type= "error"/>}
                ...
                <Container maxWidth="lg">
                    <Paper className={classes.paper}>
                        ...
                        <Box textAlign="center">
                            <Button color="secondary" variant="contained" onClick={handleRevokeClick} disabled={selected.length === 0}>Revoke</Button>
                        </Box>
                    </Paper>
                </Container>
        }
